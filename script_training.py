...
if use_cuda and paddle.device.is_compiled_with_cuda():
...
tr_optimizer = paddle.op.optim.Adam(model.parameters(), lr=learning_rate)
...
for epoch_idx in range(0, max_epoch_num):
    for batch_idx, (item, frames) in enumerate(tr_data_loader):
        ...
        if((global_ite_idx % snap_save_interval)==0):
            paddle.save(model.state_dict(), '%s/%s_snap.pt' % (saving_model_path, model_setting) )
        global_ite_idx += 1
    if((epoch_idx % save_check_interval)==0):
        paddle.save(model.state_dict(), '%s/%s_epoch_%04d.pt' % (saving_model_path, model_setting, epoch_idx) )

paddle.save(model.state_dict(), '%s/%s_epoch_%04d_final.pt' % (saving_model_path, model_setting, epoch_idx) )
